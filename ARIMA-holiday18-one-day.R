...
## fixing the station and vehicle codes
...
#### saving base forecasts results

fc.ARIMA <- NULL
for(i in 1:length(result.all)){
  fc.ARIMA <- cbind(fc.ARIMA, result.all[[i]][,1])
}
...
for(i in 1:length(mydataf)){
  ...
  for(j in 1:length(list.G)){
    fc.rec <- matrix(NA, nrow = 24, ncol = ncol(fc.ARIMA.res[[1]]))
...
fit.list <- list()
for(i in 0:0){
  fit.list[[i+1]] <- fc.ARIMA[(1+(24*i)):(24+(24*i)),]
}
fc.ARIMA.res <- list()
for(i in 0:0){
  fc.ARIMA.res[[i+1]] <- error.train.all[(1+(312*i)):(312+(312*i)),]
